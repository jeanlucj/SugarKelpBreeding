...
msX <- msX[, apply(msX, 2, function(v) !all(v == 0))]
  ...
  identical(colnames(outCovComb1)[385:628],levels(dataNHpi_RMchk$Crosses)) ####rhis= FALSE !!!Ordering of crosses in levels dataNHPi differs from outCovComb1
  
  
  ## 3. chks rows
  chkSpMat<-matrix(0, nrowchk, nrow(outCovComb1))
  dim(chkSpMat)
  #  

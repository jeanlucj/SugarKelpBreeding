#ScenarioIII Yr19,20 Predict Yr21
...
##### !!!
pop<-"Yr1920to21"
TPYr<-c(2019,2020)    # TPYr   
NAYr<-2021 

#####!!!

#### 2. Upload the dBLUPs (estimated across Yr2019 and Yr2020 TOGETHER
...
WithinYr_Both_dBLUPs<-Both_dBLUPs  # dBLUPs estimated using both years' data
...
Y1.0<-droplevels(ExpCol[ExpCol$Year%in%TPYr,])  # TPYr 2020
...
Y2.0<-ExpCol[ExpCol$Year==NAYr,]  # TPYr 2021
...
### The YRMdupPlot is the plotNo inside YdupPlotNo that is dupped within a year (2021)
YRMdupPlot<-YdupPlotCross[duplicated(YdupPlotCross[,2]),1] #first Col is the YdupPlotNo
  str(YRMdupPlot)
## These YRMdupPlot is the DupCrosses within a year, they have the same dBLUPs value, so remove one copy
Y<-Y.y<-droplevels(Y[!Y$plotNo%in%YRMdupPlot,])   # RM the dup plot within a year (2021)

## Keep Crosses and PlotNo, only for those Unique dups (no more dup within dup)
